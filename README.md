...
> **✅ GTFSの現状（8/21・差し替え完了）**：ODPT開発者登録が承認され、**アプリの時刻表は実GTFS-JP（ODPT）から生成したものに差し替え済み**（`scripts/build_gtfs.py`、対象日2026-08-22・37便）。実データの時刻は手入力時刻表と完全一致し、テストも全パス。出力は選抜停留所のみの要約で、GTFSの識別子・座標は含まない（ライセンス配慮）。ダイヤ改定時は上記1コマンドで再生成。
